...
OIDC client library can drive this flow. But three things currently have to be
configured by hand, because discovery does not advertise them:

- **PKCE with `S256` is required** at `/authorize`. `code_challenge_methods_supported`
  is absent from the discovery document, so a library configured purely from
  discovery will omit `code_challenge` and fail.
- **`client_secret_post`, not `client_secret_basic`.** RFC 7591 defaults to
  Basic, which the registration endpoint rewrites to `client_secret_post` — a
  client that then sends Basic cannot redeem codes.
- **`EdDSA`** is the only `id_token` signing algorithm.

Set those three and a generic client works. The SDK just sets them for you.
...
Note this needs the `id_token` at logout time, which the minimal Auth.js snippet
above does not retain. See the
[worked example](https://github.com/bkeyID/bkey/tree/main/examples/typescript/login-with-bkey-nextjs)
for the `jwt` callback that keeps it server-side.
