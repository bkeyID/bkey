...
`bkey.approve()` (TypeScript) or `client.approve()` (Python) starts here. Traffic to BKey is **TLS 1.2+**. BKey pushes a prompt to the phone; the phone **signs** the consent with the device's Ed25519 key (gated on facial biometrics) and returns the signature through the same channel. BKey wraps it in a JWT signed by the issuer's Ed25519 key.
