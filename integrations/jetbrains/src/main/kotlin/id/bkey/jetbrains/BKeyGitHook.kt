...
object BKeyGitHook {
    ...
    private fun buildHookScript(cliPath: String, scope: String, profile: String?): String {
        ...
        return buildString {
            ...
            appendLine()
            appendLine("# Quiet mode (default): no stderr on success. Chatty IDE-agent")
            appendLine("# watchers (Codex, Junie) sometimes classify informational stderr as")
            appendLine("# \"something went wrong\" and retry gated commits, causing duplicate")
            appendLine("# approval pushes. Only denial/error output ever goes to stderr.")
            appendLine("# Export BKEY_HOOK_QUIET=0 to restore the chatty lines.")
            appendLine("BKEY_HOOK_QUIET=\"\${BKEY_HOOK_QUIET:-1}\"")
            appendLine("_log() { [ \"\$BKEY_HOOK_QUIET\" = \"1\" ] || echo \"\$*\" >&2; }")
            appendLine()
            ...
            appendLine("fi")
            appendLine()
            appendLine("_log \"bkey hook: requesting approval for \\\"\$COMMIT_MSG\\\"…\"")
            ...
            appendLine("  _log \"bkey hook: approved.\"")
            appendLine("  exit 0")
            appendLine("fi")
            appendLine()
